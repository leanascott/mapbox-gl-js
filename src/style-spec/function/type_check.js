    ...
    if (e.literal) {
        ...
        for (let vi = 0, ti = 0; vi < argValues.length && ti < expected.args.length; ti++) {
            ...
            if (t.kind === 'nargs') {
                ...
                let j = 0;
                let argTypeNames = {};
                while (sequenceCount < t.N && vi < argValues.length) {
                    const error = match(
                        resolveTypenamesIfPossible(t.types[j], typenames),
                        argValues[vi].type,
                        argTypeNames
                    );
                    if (error) break;
                ...
                const error = match(
                    resolveTypenamesIfPossible(t, typenames),
                    argValues[vi].type,
                    typenames
                );
                if (error) errors.push({ key: argValues[vi].key, error });
...
function isGeneric (type, stack = []) {
    ...
    if (type.kind === 'typename') {
        ...
        return isGeneric(type.result) || type.args.some((t) => isGeneric(t, stack.concat(type)));
    }
    return false;
}
